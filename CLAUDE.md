...
- **`js/github.js`** — fetches the live project list from the GitHub REST API (`api.github.com/users/LuisRuiz12/repos`), filters out forks, sorts by stars then recency, caches the result in `cachedRepos`, and renders project cards into `#projectsGrid`. Re-renders from cache (no re-fetch) on `langchange`.
...
- GitHub username is hardcoded as `GITHUB_USERNAME` in `js/github.js`.
